...
tracker_names = { 'ASLA', 'BSBT', 'CPF', 'CSK', 'CT', 'CXT', 'DFT', 'FOT', 'FRAG', ...
  'IVT', 'KMS', 'L1APG', 'LOT', 'LSHT', 'LSK', 'LSS', 'MIL', 'MS', 'MTT', ...
  'OAB', 'ORIA', 'PCOM', 'PD', 'RS', 'SBT', 'SCM', 'SMS', 'STRUCK', 'TLD', ...
  'TM', 'VR', 'VTD', 'VTS' };
recompute_success_rates = false;

addpath util;
if recompute_success_rates
  sre100_results = LoadTrackingResults('SRE', 'V11_100', tracker_names);
  success_sre100 = ComputeSuccessRates(sre100_results);
  save 140919_sre100.mat sre100_results success_sre100
else
%   load 140120_sre100.mat
  load 140919_sre100.mat
end

if recompute_success_rates
  tre100_results = LoadTrackingResults('TRE', 'V11_100', tracker_names);
  success_tre100 = ComputeSuccessRates(tre100_results);
  save 140919_tre100.mat tre100_results success_tre100
else
  load 140919_tre100.mat
end

if recompute_success_rates
  ope100_results = ConvertTRE2OPE(tre100_results);
  success_ope100 = ComputeSuccessRates(ope100_results);
  save 140919_ope100.mat ope100_results success_ope100
else
  load 140919_ope100.mat
end
...
if recompute_success_rates
  ...
  save 140919_sre50.mat sre50_results success_sre50
else
  load 140919_sre50.mat
end

if recompute_success_rates
  tre50_results = LoadTrackingResults('TRE', 'V11_50', tracker_names);
  success_tre50 = ComputeSuccessRates(tre50_results);
  save 140919_tre50.mat tre50_results success_tre50
else
  load 140919_tre50.mat
end

if recompute_success_rates
  ope50_results = ConvertTRE2OPE(tre50_results);
  success_ope50 = ComputeSuccessRates(ope50_results);
  save 140919_ope50.mat ope50_results success_ope50
else
  load 140919_ope50.mat
end
...
plot_style ={ ...
  ...
  struct('Color',[0.9,0.9,0.9],'LineStyle','-','LineWidth',0.5),...% 0.8 vs 0.9
  };

set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'centimeters');
set(gcf, 'PaperPosition', [0 0 4 3] * 3.8); % last 2 are width/height.
...
%%
%- To save the figure, 'save as...' and set the type as .png or .eps (then
%- convert to pdf).
...
%- SRE results
% data = squeeze(mean(success_sre50, 2))';  test_name = 'SRE50';
% % data = squeeze(mean(success_tre50, 2))';  test_name = 'TRE50';
% % data = squeeze(mean(success_ope50, 2))';  test_name = 'OPE50';
% % data = squeeze(mean(success_sre100, 2))';  test_name = 'SRE100';
% % data = squeeze(mean(success_tre100, 2))';  test_name = 'TRE100';
% % data = squeeze(mean(success_ope100, 2))';  test_name = 'OPE100';

names = tracker_names;
attr = '';

% sre_data = success_sre50; test_name = 'SRE50'; ss = 'V11_50';
% sre_data = success_tre50; test_name = 'TRE50'; ss = 'V11_50';
% sre_data = success_ope50; test_name = 'OPE50'; ss = 'V11_50';
% sre_data = success_sre100; test_name = 'SRE100'; ss = 'V11_100';
% sre_data = success_tre100; test_name = 'TRE100'; ss = 'V11_100';
sre_data = success_ope100; test_name = 'OPE100'; ss = 'V11_100';

% weighted_average = true;
weighted_average = false;

if weighted_average
  stat_var_cmd = [test_name 'wt = sre_stat;'];
  title_str = [test_name ' (weighted average)'];
else
  stat_var_cmd = [test_name 'sq = sre_stat;'];
  title_str = [test_name ' (sequence average)'];
end
...
% fot_idx = find(strcmp('PCOM', names)); %- For debugging.
% idx(1) = fot_idx;
% sorted(1) = agg_data(fot_idx);
...
for i = numel(idx):-1:1
  j = i;
  if j > top_k, j = 11; end;
%   if j > top_k, continue; end;  %- This will only plot top-10 trackers.
...
eps_filename = sprintf('%s%s.eps', test_name, attr);
disp(['saving to ' eps_filename]);
saveas(gcf, eps_filename, 'epsc2');
...
%     results.results{k}{13} = r1.results(k);
%   end
...
% save 140921_tsre_res.mat tsre_res_15 tsre_res_30 tsre_res_60 tsre_res_90 tsre_res_120 tsre_res_150 tracker_names SEQUENCE_ANNOTATIONS tsre_results
...
%- Weighted mean
% tsre_data = tsre_res_15;  title_str = 'SRER 50 (win=15, weighted average)'; win = 15;
% tsre_data = tsre_res_30;  title_str = 'SRER 50 (win=30, weighted average)'; win = 30;
% tsre_data = tsre_res_60;  title_str = 'SRER 50 (win=60, weighted average)'; win = 60;
% tsre_data = tsre_res_90;  title_str = 'SRER 50 (win=90, weighted average)'; win = 90;
% tsre_data = tsre_res_120;  title_str = 'SRER 50 (win=120, weighted average)'; win = 120;
tsre_data = tsre_res_150;  title_str = 'SRER 50 (win=150, weighted average)'; win = 150;
...
hold on;
top_names = cell(numel(idx), 1);
for i = 1:top_k  %- To keep the legends correct.
  plot(data(:,idx(i),2), data(:,idx(i),1), '.-', ...
    'Color', plot_style{i}.Color, 'LineWidth', 1.5, 'MarkerSize', 15);
...
eps_filename = sprintf('SRER%dwin%d%s.eps', numel(seqs), win, attr);
disp(['saving to ' eps_filename]);
saveas(gcf, eps_filename, 'epsc2');
end
