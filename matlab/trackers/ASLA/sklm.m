...
%   incremental SVD algorithm   :   增量SVD算法
...
%
%  required input
%    data (N,n) : initial/additional data			初始或新加数据
%    U0 (N,d) : old basis                           旧的基向量
%    D0 (d,1) : old singular values                 旧的特征值
%  optional input
%    mu0 (N,1) : old mean                           旧的均值
%    n0 : number of previous data                   之前的样本个数
%    ff : forgetting factor (def=1.0)               遗忘因子
%    K : maximum number of basis vectors to retain	最大基向量个数
%
%  output
%    U (N,d+n) : new basis                          新的基向量		
%    D (d+n,1) : new singular values                新的特征值
%    mu (N,1) : new mean                            新的均值
%    n : new number of data                         目前的样本个数
