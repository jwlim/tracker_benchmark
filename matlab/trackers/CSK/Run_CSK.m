...
function results = Run_CSK(imgfilepath_fmt, img_range_str, init_rect, run_opt)
...
positions = zeros(num_frames, 2);  %to calculate precision
rect_position = zeros(num_frames, 4);

for frame = 1:num_frames,
  %load image
  im = imread(sprintf(imgfilepath_fmt, img_range(frame)));
  if size(im,3) > 1,
    im = rgb2gray(im);
  end
  if resize_image,
  ...
  if ~isempty(run_opt.dumppath_fmt)
    PlotResultRect(im, img_range(frame), rect_position(frame,:), run_opt.dumppath_fmt);
  end
%   if bSaveImage
