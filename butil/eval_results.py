...
def calc_result(m, tracker, seqs, results, evalType):
    ...
    for j in range(idxNum):
        ...
        if evalType == 'TRE':
            anno = seq.subAnno[j]
        try:
            aveCoverage, aveErrCenter, errCoverage, errCenter = m.calcSeqErrRobust(result, matlab.double(anno), nargout=4)
            seq.aveCoverage.append(aveCoverage)
            seq.aveErrCenter.append(aveErrCenter)
            seq.errCoverage += errCoverage
            seq.errCenter += errCenter
        except:
            print "calcSeqErrRobust failed for '{0}', {1}/{2}".format(seq.name, len(anno), len(result['res']))
        ...
        for seq in seqs:
            if attr.name in seq.attributes:
                ...
                for threshold in thresholdSetOverlap:
                	seqSuccess = [score[0] for score in seq.errCoverage 
                ...
                overlapList = [score[0] for score in seq.errCoverage 
                    if score[0] > 0]
                ...
                THRESHOLD = 0.5
                errorNum = len([score[0] for score in seq.errCoverage 
                    if score[0] < THRESHOLD]) / float(length) * 10
